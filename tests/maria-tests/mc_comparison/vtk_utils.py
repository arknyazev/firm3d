...
Point-cloud writer (``write_points_vtu``) and polyline writer
(``write_polylines_vtu``) are line-for-line equivalent to the inline XML
helpers in ``mc_backward/backward_informed_mc.py`` — consolidating them here
so the three MC-comparison scripts import rather than duplicate.

``trace_snapshots`` sweeps ``tmax`` from ``tmax/n`` to ``tmax`` across
``n_snapshots`` re-runs of the tracer so a single particle's trajectory
through the drag integrator can be reconstructed.  The backward-drag and
forward-drag GPU tracers share the same calling convention, so one helper
handles both: pass ``cartesian_gpu_tracing_drag`` or
``cartesian_gpu_tracing_backward_drag`` as ``tracer``.  Each snapshot call is
one fresh GPU run — cost scales as ``n_snapshots * n_trajectory``.
"""
import time
...
def write_coils_and_surface_vtk(out_dir, curves, s_input,
    ...
    """Write the coil curves and the LCFS surface in Paraview-compatible VTK.

    Produces ``<coils_stem>.vtu`` and ``<surface_stem>.vts`` under ``out_dir``
    (extensions are added by the simsopt writers)."""
...
def write_points_vtu(filename, xyz, point_data=None):
    """Write a point cloud as an UnstructuredGrid with one VTK_VERTEX cell per
    point.  ``point_data`` is an optional dict of per-point Float64 arrays."""
    npts = len(xyz)
    if npts == 0:
...
def write_polylines_vtu(filename, pts_per_poly, pts,
    ...
    """Write ``n_poly = pts.shape[0]`` polylines of ``pts_per_poly`` points
    each as VTK_POLY_LINE cells.

    ``pts`` : (n_poly, pts_per_poly, 3) Float64
    ``point_data`` : dict[str, (n_poly, pts_per_poly)] -> per-point Float64.
    ``cell_data``  : dict[str, (n_poly,)]             -> per-cell  Int32.
    """
    n_poly = pts.shape[0]
    if n_poly == 0:
...
def trace_snapshots(tracer, field, R_init, phi_init, Z_init, vpar_init,
    ...
    Returns (xyz[N,n_snap,3], vpar[N,n_snap], H[N,n_snap],
             time[N,n_snap], stop[N,n_snap], tmax_snaps[n_snap]).

    ``use_energy_stop`` should be True for the backward tracer (so a particle
    that reached H_fusion at snapshot i stays frozen at that endpoint for
    later snapshots — which is the behaviour that makes polylines meaningful)
    and False for the forward tracer.
    """
    n = len(R_init)
...
def write_trajectory_polylines(filename, initial_xyz, snap_xyz,
                               ...
                               particle_ids=None):
    """Assemble (initial + snapshots) polylines and dispatch to
    ``write_polylines_vtu``."""
